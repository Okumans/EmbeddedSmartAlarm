...
#### Step 2.3: Generate Presigned URL
- Creates presigned URL for secure, temporary access
- **Expiration**: 7 days (604,800 seconds)
- ESP32 can download directly from MinIO without authentication
- **Format**: `http://172.20.10.2:9000/audio/esp32-audio/sound_{soundId}.mp3?X-Amz-Algorithm=...&X-Amz-Credential=...&X-Amz-Expires=604800...`
- Progress: **50%**
...
- **Message Format**: `{presignedUrl}|{soundId}`
- **Example**: `http://172.20.10.2:9000/audio/esp32-audio/sound_EbBW3i3L.mp3?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=...&X-Amz-Expires=604800...|EbBW3i3L`
- **URL Expiration**: 7 days from generation
...
- Parse message: Split by `|` character
  - `parts[0]`: Presigned download URL (with authentication parameters)
  - `parts[1]`: Sound ID
...
void callback(char* topic, byte* payload, unsigned int length) {
  if (strcmp(topic, "esp32/audio_download_cmd") == 0) {
    ...
    String presignedUrl = message.substring(0, separatorIndex);
    String soundId = message.substring(separatorIndex + 1);
    
    downloadAudio(presignedUrl, soundId);
  }
}
```

#### Step 3.3: HTTP Download
- Make HTTP GET request to the presigned URL
- No authentication headers needed (credentials in URL query parameters)
- Save MP3 file to ESP32 storage (SPIFFS/SD card)
- Filename: `sound_{soundId}.mp3`
- **Note**: URL expires after 7 days, ESP32 should download immediately

**Example C++ Code**:
```cpp
void downloadAudio(String presignedUrl, String soundId) {
  HTTPClient http;
  http.begin(presignedUrl);  // URL already includes authentication
...
|-------|-----------|-----------|------------|----------------|
| `esp32/audio_download_cmd` | Backend → ESP32 | Backend API | ESP32 | `{presignedUrl}\|{soundId}` |
...
**Note**: The presigned URL includes authentication query parameters and expires after 7 days.
...
| Presigned URL Generated | 50% | Secure 7-day download URL created |
...
- Must reach presigned URL (includes query parameters for authentication)
- Must reach `broker.hivemq.com:1883` for MQTT
- Requires WiFi connection
- **Note**: Presigned URLs expire after 7 days, download should happen immediately
...
Presigned URL generated for ESP32 (7 days expiration)
[MQTT Publish Success] Topic: esp32/audio_download_cmd, Command: {presignedUrl}|{soundId}
